...
export default function NewOrderPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (oErr) throw new Error(`Order failed: ${oErr.message}`)

      // 2. Upload files one by one
      const uploadedPaths: string[] = []
      for (let i = 0; i < files.length; i++) {
        const f    = files[i]
        const path = `${user.id}/${order.id}/${i}_${f.name.replace(/[^a-zA-Z0-9._-]/g, '_')}`
        const { error: upErr } = await sb.storage.from('originals').upload(path, f, { upsert: true })
        if (upErr) throw new Error(`Upload failed for ${f.name}: ${upErr.message}`)
        uploadedPaths.push(path)
        setProgress(Math.round(((i + 1) / files.length) * 100))
      }

      // 3. Save file paths to order
      const { error: updErr } = await sb.from('orders').update({
        original_urls: uploadedPaths,
            ...
            <div>
              <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', padding: 24, marginBottom: 20 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  {SERVICES.map(s => (
                    <button key={s.id} onClick={() => setService(s.id)} style={{ textAlign: 'left', padding: 16, borderRadius: 10, border: `2px solid ${service === s.id ? '#2563eb' : '#e5e7eb'}`, background: service === s.id ? '#eff6ff' : '#fff', cursor: 'pointer' }}>
              ...
              <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', padding: 20, marginBottom: 20 }}>
                ...
                <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="e.g. keep the shadow, specific cropping instructions..." style={{ width: '100%', padding: '10px 12px', border: '1.5px solid #d1d5db', borderRadius: 8, fontSize: 14, resize: 'vertical', minHeight: 80, outline: 'none', fontFamily: 'Inter, sans-serif' }} />
              </div>
          ...
          {step === 2 && (
            <div>
              <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', padding: 20, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <button onClick={() => setStep(1)} style={{ marginLeft: 'auto', fontSize: 12, color: '#2563eb', background: 'none', border: 'none', cursor: 'pointer' }}>Change</button>
              </div>
              ...
              <div
                ...
                style={{ border: `2px dashed ${dragOver ? '#2563eb' : '#d1d5db'}`, borderRadius: 12, padding: 48, textAlign: 'center', cursor: 'pointer', background: dragOver ? '#eff6ff' : '#fff', marginBottom: 16 }}
              >
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                <button onClick={() => setStep(1)} style={{ padding: '13px 20px', borderRadius: 10, border: '1.5px solid #d1d5db', background: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer', color: '#374151' }}>← Back</button>
            ...
            <div>
              <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', padding: 28, marginBottom: 20 }}>
                ...
                {[
                  ...
                  ['Photos', `${files.length} images`],
                  ['Price per image', `£${selectedService?.price}`],
                  notes ? ['Notes', notes] : null,
                ].filter(Boolean).map(([label, value]: any) => (
                  <div key={label} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
                    <span style={{ fontSize: 14, color: '#6b7280' }}>{label}</span>
                    <span style={{ fontSize: 14, fontWeight: 600, color: '#111', maxWidth: 300, textAlign: 'right' }}>{value}</span>
                  </div>
                ))}
              ...
              <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 10, padding: '14px 18px', marginBottom: 20, fontSize: 13, color: '#92400e' }}>
                💡 After placing your order, our team will process your photos within 24–48 hours and notify you when ready to review.
              </div>

              {uploading && (
                <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #e5e7eb', padding: 18, marginBottom: 16 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                    <span style={{ fontSize: 14, fontWeight: 600 }}>Uploading photos...</span>
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                <button onClick={() => setStep(2)} disabled={uploading} style={{ padding: '13px 20px', borderRadius: 10, border: '1.5px solid #d1d5db', background: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer', color: '#374151' }}>← Back</button>
